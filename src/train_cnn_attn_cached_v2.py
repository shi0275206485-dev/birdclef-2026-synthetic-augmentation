"""
Group A - sub-experiment 2: CNN + Attention Pooling, trained by reading pre-cached mel spectrograms from the G drive.
Must first run precompute_melspec.py to generate the cache.

Reading the cache is much faster than computing spectrograms on the fly, and it's not recomputed every epoch.
Windows + GPU safe.
"""
...
CACHE_DIR  = r"G:\bird_melcache_v2"    # new cache aligned with champion parameters

N_SEG       = 6
BATCH_SIZE  = 32       # drop to 8 on OOM
EPOCHS      = 150       # raised the upper limit, let early stopping decide when to stop (previously artificially capped at 30)
LR          = 1e-2
PATIENCE    = 25        # more patience, to avoid falsely triggering early stop from fluctuations during LR decay
NUM_WORKERS = 10
DROP_OUT = 0.4
...
class CachedDataset(Dataset):
    def __init__(self, df, train=True):
        ...
        # keep only entries whose cache exists
...
def main():
    ...
    # Split: singleton classes (recording-level count<2) are forced into the validation set (assignment requirement), the rest split 80/20 by filename
    y_arr = df["y"].values
    ...
    print(f"Singleton classes (forced into validation set): {len(rare)} classes, {len(df_rare)} recordings")
    print(f"Classes covered in training set: {df_train['y'].nunique()}/{num_class}")

    train_ds = CachedDataset(df_train, train=True)
    val_ds   = CachedDataset(df_val, train=False)
    print(f"Train {len(train_ds)}, val {len(val_ds)} (with cache)")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        print(f"Epoch {ep:2d} | train_loss {tl:.4f} | val_loss {vl:.4f} | "
    ...
    # ===== Confusion matrix (first 30 classes, otherwise 206 classes are too dense to read) =====
