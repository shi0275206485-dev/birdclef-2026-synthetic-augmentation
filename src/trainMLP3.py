...
LR           = 1e-3
WEIGHT_DECAY = 1e-3          # L2 regularization (strengthened from the 0.3 version's 1e-3 to curb overfitting)
DROPOUT      = 0.5           # 0.3 -> 0.5, the main regularizer, to keep val loss from rising
PATIENCE     = 10            # early stopping: stop if val macro-F1 hasn't improved for N consecutive epochs
HIDDEN       = [256, 128]    # 512,256 -> 256,128, smaller capacity makes it harder to memorize noise
...
def evaluate(loader):
    ...
    for xb, yb in loader:
        ...
        logits_b = model(xb)
        loss_sum += criterion(logits_b, yb_dev).item() * xb.size(0)
        n += xb.size(0)
...
for epoch in range(1, EPOCHS + 1):
    ...
    yt, pred, _, val_loss = evaluate(val_loader)
...
yt, pred, prob, _ = evaluate(val_loader)
...
# =====================================================
# CURVES  (split into two figures: loss plot + validation metrics plot)
# =====================================================

epochs_ran = range(1, len(history["train_loss"]) + 1)

# ---- Figure 1: Loss (train loss + val loss, same units, single axis) ----
plt.figure(figsize=(10, 5))
plt.plot(epochs_ran, history["train_loss"], color="tab:blue", label="train loss")
plt.plot(epochs_ran, history["val_loss"], color="tab:red", label="val loss")
plt.xlabel("Epoch")
...
# ---- Figure 2: Validation metrics (val acc + val macro F1, both 0-1 metrics, single axis) ----
plt.figure(figsize=(10, 5))
plt.plot(epochs_ran, history["val_acc"], color="tab:green", label="val accuracy")
plt.plot(epochs_ran, history["val_macro_f1"], color="tab:orange", label="val macro F1")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.title("MLP Validation Metrics (segment-level)")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("mlp_val_metrics.png", dpi=120)
plt.show()
