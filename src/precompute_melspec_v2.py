"""
Preprocessing: convert each recording into N_SEG mel-spectrogram segments, cached to the G drive.
Only needs to run once. Afterward, training scripts read the cache directly, no longer recomputing spectrograms every epoch.

Each recording is stored as one .npy: shape (N_SEG, n_mels, time)
About 35000 recordings x ~1MB each ~= 33GB, the G drive (800G) has plenty of room.
...
CACHE_DIR   = r"G:\bird_melcache_v2"     # new directory (parameters changed, old cache invalidated)
N_WORKERS   = 6
...
N_MELS      = 128
N_FFT       = 2048      # 1024 -> 2048 (aligned with champion, higher frequency resolution)
HOP         = 512
FMIN        = 20        # new: cut off 0-20Hz low-frequency noise
FMAX        = 16000     # explicitly stated (= sr/2)
N_SEG       = 6
...
def process_one(fname):
    ...
    # reproducible for validation: evenly sample N_SEG segments (randomness for training is handled separately as augmentation in the training script)
    if n_avail >= N_SEG:
